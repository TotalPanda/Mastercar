...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // MCar: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "MasterCar Signed Message:\n";
...
{
    // MCar: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    {
    nSubsidy = 40 * COIN; //20000 blocks x40 ==> 800,000 MCAR
    }
     else if(nHeight < 100105) 
    {
    nSubsidy = 10 * COIN; //80000 blocks x10 ==> 800,000 MCAR
    }
     else if(nHeight < 200106) 
    {
    nSubsidy = 4 * COIN; //100000 blocks ==> 400,000 MCAR
    }
     else if(nHeight < 300107) 
    {
    nSubsidy = 2 * COIN; //100000 blocks ==> 200,000 MCAR
    } 
    else if(nHeight < 301108) 
    {
    nSubsidy = 1 * COIN; //1000 blocks ==> 1,000 MCAR
    }
    else if(nHeight < 301109) 
    {
    nSubsidy = 1337 * COIN; //"EXTRA LUCKY BLOCK" == 1337 MCAR
    }
    else if(nHeight < 302110) 
    {
    nSubsidy = 10 * COIN; //1000 blocks x10 ==> 10000 MCAR
    }
    else if(nHeight < 303111) 
    {
    nSubsidy = 2 * COIN; //1000 blocks x10 ==> 2000 MCAR
    }
    // 302110 blocs = MINER's TIME ==> approx. 3 mining months [supply = 2,291,837 coins] 
    else if(nHeight < 1303112) 
    {
    nSubsidy = 0.7 * COIN; //1,000,000 blocks x0.1 ==> 700,000 MCAR
    }
...
static const int64 nTargetTimespan = 5 * 30; // MasterCar : diff retargeting every 5 blocks
static const int64 nTargetSpacing = 1 * 30; // MasterCar : 30s block ==> 288 block per day
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // MCar: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // MCar: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // MCar: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // MasterCar: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("MasterCarMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("MasterCarMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static MasterCarMiner(CWallet *pwallet)
{
    printf("MasterCarMiner:\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mastercar-miner");
    ...
    try { loop {
        ...
        printf("Running MasterCar with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("MasterCarMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MasterCarMiner, pwallet));
}
